...
public class Movies implements IMovies{
    ...
    // main store 
    private MyHashMap<Integer, Movie> movies;

    // secondary stores 
    private MyHashMap<Genre, MyHashSet<Integer>>   genreInd;
    private MyHashMap<Integer, MyHashSet<Integer>> collectionInd;
    private MyRBTree<LocalDate, MyHashSet<Integer>> releaseDateInd;


    // collection data
    private MyHashMap<Integer, CollectionInfo> collections;

    public Movies(Stores stores) {
        this.stores = stores;
        this.movies = new MyHashMap<>();
        this.genreInd = new MyHashMap<>();
        this.collectionInd = new MyHashMap<>();
        this.releaseDateInd = new MyRBTree<>();
    ...
    private static class Movie {
        String title, originalTitle, overview, tagline, status, homepage, poster, originalLanguage, imdbID;
        long budget, revenue;
        double runtime, popularity, voteAverage;
        ...
        Movie(String title, String originalTitle, String overview, String tagline,
    ...
    public boolean add(int id, String title, String originalTitle, String overview, String tagline, String status, Genre[] genres, LocalDate release, long budget, long revenue, String[] languages, String originalLanguage, double runtime, String homepage, boolean adult, boolean video, String poster) {
        ...
        Movie m = new Movie(title, originalTitle, overview, tagline, status, genres,
        ...
        for (Genre g : genres) {
            if (!genreInd.containsKey(g)) genreInd.put(g, new MyHashSet<>());
            genreInd.get(g).add(id);
    ...
    public boolean remove(int id) {
        ...
        if (m.collectionID != -1) {
            MyHashSet<Integer> colSet = collectionInd.get(m.collectionID);
            if (colSet != null) colSet.remove(id);
        }
    ...
    public String getOriginalTitle(int id) {
        Movie m = movies.get(id); 
        return m == null ? null : m.originalTitle;
    }
    ...
    public String getOriginalLanguage(int id) {
        Movie m = movies.get(id); 
        return m == null ? null : m.originalLanguage;
    }
